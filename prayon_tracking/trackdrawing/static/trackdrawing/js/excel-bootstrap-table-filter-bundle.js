$(document).ready(function() {
    $('.excelFilter').excelTableFilter();
    $(".dropdown-filter-menu-search").on("change paste keyup", function() {
        if( $(this).val().length>0){
            $(this).closest('th').find('img').attr("src","img/filter_full.svg");
        }else{
            $(this).closest('th').find('img').attr("src","img/filter_empty.svg");
     ...
     $(".dropdown-filter-menu-item").change(function() {
         var allcheck = 0;
        if ($('.dropdown-filter-menu-item:hover').length != 0){
            if(!this.checked) {
                $(this).closest('th').find('img').attr("src","img/filter_full.svg");
            }else{
                $(this).closest('th').find('img').attr("src","img/filter_empty.svg");
                var size = $(this).closest('th').find('input[type=checkbox]').length;
               $(this).closest('th').find('input[type=checkbox]').each(function (index) {
    ...
    if($( ".noFilter" ).length){
        $( ".noFilter" ).find(".dropdown-filter-dropdown").remove();
    }
});
...
var FilterMenu = function () {
    ...
    FilterMenu.prototype.dropdownFilterItem = function (td, self) {
        ...
        input.type = 'checkbox';
        input.value = value.trim().replace(/ +(?= )/g, '').toString();
    ...
    FilterMenu.prototype.dropdownFilterItemSelectAll = function () {
        ...
        var input = document.createElement('input');
        input.type = 'checkbox';
        input.value = this.options.captions.select_all.toString();
        input.setAttribute('checked', 'checked');
    ...
    FilterMenu.prototype.dropdownFilterDropdown = function () {
        ...
        dropdownFilterDropdown.setAttribute("id", "filterID");
        ...
        icon.setAttribute("class", "filter-img");
        icon.setAttribute("src", "img/filter_empty.svg");
