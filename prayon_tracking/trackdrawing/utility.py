...
def Modify_drawing_status_from_csv():
    ...
    with open('D:\\sapToRecheckLiasse2.csv', 'r') as file:
        ...
        for row in reader:
            ...
            recordSAP.save()
            print('Workdata id ' + str(record_to_modify.pk) + '(' + str(
                recordSAP.pk) + ')' + ' status changed from ' + old_status + ' to re-check')

            SAP_log.info(
                'Workdata id ' + str(record_to_modify.pk) + '(' + str(
                    recordSAP.pk) + ')' + ' status changed from ' + old_status + ' to re-check')
...
def Modify_Multipages_drawing_from_csv():
    ...
    with open('D:\\RecordToDuplicate.csv', 'r') as file:
        ...
        for row in reader:
            ...
            log_info("Record " + cadastre + "(" + str(
                record_to_duplicate.pk) + "), cadastre set to " + record_to_duplicate.num_cadastre, 'CreateDuplicate')
            ...
            for page in range(1, int(row[1])):
                ...
                record_to_duplicate.save()
                log_info("Record " + cadastre + " copy to id " + str(
                    record_to_duplicate.pk) + " , cadastre set to " + record_to_duplicate.num_cadastre,
                         'CreateDuplicate')
        ...
        for row in reader:
            ...
            if old_num != record.old_num:
                ...
                log_info("Record " + record.num_cadastre + "(" + str(
                    record.pk) + "), ancien plan changed to " + old_num + "(" + record.old_num + ")",
                         'BatchModification')
                record.old_num = old_num
                record.remark = "Ancien plan: " + old_num
            if ordre != record.ordre:
                # change Num ordre
                log_info("Record " + record.num_cadastre + "(" + str(record.pk) + "), ordre changed to " + str(
                    ordre) + "(" + str(record.ordre) + ")", 'BatchModification')
                record.ordre = ordre
            if num_imp != record.num_imp:
                # change Num imputation
                log_info("Record " + record.num_cadastre + "(" + str(
                    record.pk) + "), imputation changed to " + num_imp + "(" + record.num_imp + ")",
                         'BatchModification')
                record.num_imp = num_imp
            if num_trav != record.num_trav:
                # change Num travaux
                log_info("Record " + record.num_cadastre + "(" + str(
                    record.pk) + "), Num Trav changed to " + num_trav + "(" + record.num_trav + ")",
                         'BatchModification')
...
def create_new_drawing_id():
    records = ExtractSAP.objects.all().exclude(
        status='BACKLOG')  # Tous les enregistrements sauf ceux qui sont au Backlog
    ...
    records = records_multi.filter(
        num_cadastre__regex=r'(F\d{3})$')  # Conserve uniquement les enregistrements multipage
    list_test = list(records.values_list('site', 'div', 'ordre', 'typ__code').distinct())
...
def Modify_Num_Cadastre_from_csv():
    ...
    with open('D:\\3_4_duplicate.csv', 'r') as file:
        ...
        for row in reader:
            ...
            log_info("Record " + cadastre + "(" + str(
                record_to_duplicate.pk) + "), cadastre set to " + record_to_duplicate.num_cadastre, 'ModifBase200602')
            # os.rename(os.path.join(fs.location, filename),
            #           os.path.join(os.path.join(fs.location, 'backup'), filename))
            record_to_duplicate.save()
            for page in range(1, int(row[1])):
    ...
    with open('D:\\1_2_duplicate.csv', 'r') as file:
        ...
        for row in reader:
            ...
            if record_to_update:
                ...
                record.file_exists = ''
                record.remark = ''
                log_info("Record " + cadastre + "(" + str(record.pk) + "), cadastre set to " + new_cadastre,
                         'ModifBase200602')
                record.save()
                owork.delete()
            if record_to_duplicate:
                ...
                record.status = ''
                record.save()
                log_info("Record " + cadastre + " copy to id " + str(record.pk) + " , cadastre set to " + new_cadastre,
                         'ModifBase200602')


def Maj20200604():
    import csv
    with open('D:\\Maj0406.csv', 'r') as file:
        reader = csv.reader(file, delimiter='|')
        for row in reader:
            pk, division_ausy, div = row
            try:
                owork = Work_data.objects.get(id=pk)
            except Work_data.DoesNotExist:
                log_info(
                    "Work_data object with id " + pk + " does not exists",
                    'ModifBase200604')
                owork = None
            if owork:
                record = ExtractSAP.objects.get(id=owork.id_SAP.pk)
                if division_ausy:
                    if division_ausy != record.division_ausy:
                        record.division_ausy = division_ausy
                        record.save()
                        log_info(
                            "Record " + record.num_cadastre + " (" + str(
                                record.pk) + "), Division AUSY set to " + division_ausy,
                            'ModifBase200604')
                if div:
                    record.div = div
                    record.save()
                    log_info(
                        "Record " + record.num_cadastre + " (" + str(record.pk) + "), Division set to " + div,
                        'ModifBase200604')
    file.close()
    pk_split = [31846, 31884, 31886, 31887, 31890, 31902, 31904, 31905, 31847, 31885, 31906, 31888, 31889]
    for pk in pk_split:
        record = ExtractSAP.objects.get(id=Work_data.objects.get(id=pk).id_SAP.pk)
        titre = record.title.split('|')
        record.division_ausy = titre[0]
        record.title = titre[1]
        record.save()
        log_info(
            "Record " + record.num_cadastre + " (" + str(record.pk) + "), Division set to " + titre[0],
            'ModifBase200604')
        log_info(
            "Record " + record.num_cadastre + " (" + str(record.pk) + "), Title set to " + titre[1],
            'ModifBase200604')
    ...
    for record in ExtractSAP.objects.all():
        log_info("Record " + record.num_cadastre + "(" + str(record.pk) + "), ordre nouveau changed to " + str(
            record.ordre) + "(" + str(record.ordre_nv) + ")", 'ModifBase200604')
        record.ordre_nv = record.ordre
        record.save()
        if record.div:
            if int(record.div) in div_dict:
                log_info("Record " + record.num_cadastre + "(" + str(record.pk) + "), ordre changed to " + str(
                    div_dict[int(record.div)]) + "(" + str(record.ordre) + ")", 'ModifBase200604')
                record.ordre = div_dict[int(record.div)]
                record.save()
            else:
                log_info("Record " + record.num_cadastre + "(" + str(record.pk) + "), pas d'ordre pour div = " +
                    record.div, 'ModifBase200604')
        elif record.ordre:
            log_info("Record " + record.num_cadastre + "(" + str(record.pk) + "), ordre set to None (" + str(
                record.ordre) + ")", 'ModifBase200604')
            record.ordre = None
            record.save()
